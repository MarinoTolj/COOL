...
int main()
{
	...
	for (auto it : files)
	{
		...
		do
		{
			...
			// comm = "./codegen <" + filename_results + ".sm >" + filename_results + ".ll";
			comm = "./lexer " + filename + ".cl | ./parser " + filename + ".cl | ./semant " + filename + ".cl | ./codegen " + filename + ".cl >" + filename_results + ".ll";
			system(comm.c_str());
			// proveravam je li došlo do greške u parsiranju AST-a
		} while (!doesFileExists(filename_results + ".ll"));

		// string sa naredbom za spim i izlazom i stderr u datoteku
		// comm = "spim -file " + filename_results + ".s >" + filename_results + ".spim.myout 2>&1";
		// system(comm.c_str());
		comm = "lli " + filename_results + ".ll >" + filename_results + ".spim.myout 2>&1";
		system(comm.c_str());
		...
		do
		{
			...
			{
				ok = false;
				diff << "error in line# " << lineno << "\n";
				diff << "    parsed as: " << mystr1 << "\n";
				diff << "    should be: " << str1 << "\n";
